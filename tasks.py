...
class TravelTasks:
    ...
    def plan_itinerary(self, agent, city, travel_dates, interests, currency, budget):
        return Task(
            description=dedent(
                f"""
                ...
                **Description**: Expand the city guide into a full 7-day travel itinerary with detailed 
                    ...
                    Also if travelling to a foreign country where the currency mentioned is not used show the exchange rate
            ...
            agent=agent,
            expected_output="A detailed 7-day travel itinerary with attractions, hotels, restaurants, budget, and packing suggestions."
        )

    def identify_city(self, agent, origin, cities, interests, travel_dates, currency, budget):
        return Task(
            description=dedent(
                f"""
            ...
            agent=agent,
            expected_output="A detailed report on the selected city, including flight costs, weather, attractions, and events."
        )

    def gather_city_info(self, agent, city, travel_dates, interests,currency,budget):
        return Task(
            description=dedent(
                f"""
            ...
            agent=agent,
            expected_output="A comprehensive city guide including attractions, events, weather, and costs."
        )
